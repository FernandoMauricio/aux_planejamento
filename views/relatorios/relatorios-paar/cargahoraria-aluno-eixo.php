...
?>
           ...
           </strong>Carga Horária Aluno por Eixos Tecnol&oacute;gicos</font></td>
					  ...
					  $query_eixos2 = "SELECT eix_codeixo, eix_descricao FROM eixo_eix WHERE eix_codeixo <> 8 ORDER BY eix_descricao";
			           ...
			           foreach ($eixos2 as $eixo2) {
			           	...
			           	$codigo_eixo = $eixo2['eix_codeixo'];
							...
							$quantidade_cargahoraria_por_eixo = 0;
							 //EXTRAINDO AS QUANTIDADES CONFORME O EIXO E UNIDADE....
				        $query_cargahorariaEixo = "SELECT placu_quantidadeturmas, placu_quantidadeturmas,placu_quantidadealunos,placu_quantidadealunosisentos,placu_quantidadealunospsg, placu_cargahorariaarealizar, placu_cargahorariavivencia FROM planilhadecurso_placu WHERE placu_codsituacao = '".$situacao_planilha['sipla_codsituacao']."' AND placu_codano = '".$ano_orcamento['an_codano']."' AND placu_codtipla = '".$tipo_planilha['tipla_codtipla']."' AND placu_codunidade = '".$codigo_unidade."' AND placu_codeixo = '".$codigo_eixo."'";

				        $cargaHorariaEixos = Planilhadecurso::findBySql($query_cargahorariaEixo)->all(); 

			            foreach ($cargaHorariaEixos as $cargaHorariaEixo) {

					        $quantidade_turmas 			      = $cargaHorariaEixo['placu_quantidadeturmas'];
						    $quantidade_alunos                = $cargaHorariaEixo['placu_quantidadealunos'];
						    $quantidade_alunos_psg            = $cargaHorariaEixo['placu_quantidadealunospsg'];
						    $quantidade_alunos_isentos        = $cargaHorariaEixo['placu_quantidadealunosisentos'];
						    $quantidade_cargahoraria          = $cargaHorariaEixo['placu_cargahorariaarealizar'];
						    $quantidade_cargahoraria_vivencia = $cargaHorariaEixo['placu_cargahorariavivencia'];
					               
								$quantidade_cargahoraria = $quantidade_cargahoraria + $quantidade_cargahoraria_vivencia;
								$soma_matricula          = $quantidade_alunos + $quantidade_alunos_psg + $quantidade_alunos_isentos;
								
								$quantidade_cargahoraria_por_eixo += $quantidade_turmas * $soma_matricula * $quantidade_cargahoraria;
								$soma_total_geral                 += $quantidade_turmas * $soma_matricula * $quantidade_cargahoraria;
					        }// FIM DAS QUANTIDADES..
					        
							$conta_eixo[$codigo_eixo] += $quantidade_cargahoraria_por_eixo;
							$subtotal_unidade         += $quantidade_cargahoraria_por_eixo;
